...
from PyQt5.QtWidgets import QApplication, QDesktopWidget, QLineEdit, QWidget
...
class SpotlightUI(QWidget):
    ...
    def __init__(self, sp: Spotify, song_queue: SongQueue, parent=None):
        ...
        # theming and style
        self.active_theme = default_themes[
            "dark"
        ]  # TODO Use Properties Class to get theme when implemented
        ...
        )  # TODO Replace with properties class reference when implemented
    ...
    def create_widgets(self):
        self.resize(
            540, self.small_row_height + self.standard_row_height
        )  # makes up the height of the widget
    ...
    def eventFilter(self, source, event):
        ...
        if (
            ...
            hide()
        if event.type() == QtCore.QEvent.FocusOut and not any(
            (w.hasFocus() if w is not None else False) for w in self.children()
        ):  # hides if not focused
            hide()
    ...
    def add_row(self, index, suggestion):
        ...
        self.rows[index].clicked.connect(
            lambda: self.suggestion_exe_handler(suggestion)
        )
        ...
        if self.function_row.isHidden():
            ...
            self.rows[index].move(
                0, self.standard_row_height * (1 + index) + self.small_row_height
            )
    ...
    def suggestion_exe_handler(self, suggestion):
        if suggestion.setting == "fill":
            ...
            self.textbox.setText(
                suggestion.fill_str
            ) if suggestion.setting == "menu_fill" else None
