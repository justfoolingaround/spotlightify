...
from spotlight.suggestions.playable.song import QueueSuggestion, SongSuggestion
        ...
        if parameter != "":
            try:
                if self.type_ == "queue" or self.type_ == "song":
                    ...
                    results = self.sp.search(q=parameter, limit=10, type=self.type_)[
                        f"{self.type_}s"
                    ]["items"]
                ...
                if len(item_list) == 0:
                    item_list = [
                        WarningSuggestion(
                            "No Results Found", "Please adjust search term."
                        )
                    ]
                else:
                ...
                item_list = [
                    WarningSuggestion("No Results Found", "Please adjust search term")
                ]
        return item_list
