...
class GoToCommand(Command):
    def __init__(self):
        Command.__init__(self, "Seek", "Seek to a point in a song e.g. '2:10'", "seek ")

    def get_suggestions(self, parameter=""):
        if parameter == "":
            return [FillSuggestion(self.title, self.description, "forward", "seek ")]
        else:
            return [
                ExecutableSuggestion(
                    f"{self.title} {parameter}",
                    self.description,
                    "forward",
                    PlaybackManager.goto,
                    parameter=parameter,
                )
            ]
        ...
        if parameter != "":
            ...
            return [
                ExecutableSuggestion(
                    ...
                    self.description,
                    "share",
                    PlaybackManager.copy_url_to_clipboard,
                )
            ]
...
class AuthenticationCommand(Command):
    ...
    def get_suggestions(self, parameter=""):
        if parameter != "":
            ...
            return [
                ExecutableSuggestion(self.title, self.description, "cog", lambda: None)
            ]  # Special Case
...
class ExitCommand(Command):
    ...
    def get_suggestions(self, parameter=""):
        if parameter != "":
            ...
            return [
                ExecutableSuggestion(
                    self.title, self.description, "exit", PlaybackManager.exit_app
                )
            ]
